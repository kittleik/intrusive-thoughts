...
router.get('/night-stats', (_req, res) => {
  try {
    ...
    // Read night_summary.json (generated by night_summary.py)
    const summaryPath = path.join(scriptDir, 'night_summary.json');
    let nightSummary: any = {};
    try {
      nightSummary = JSON.parse(fs.readFileSync(summaryPath, 'utf-8'));
    } catch { /* no summary */ }
    
    // Also check history for night entries (03:00-07:00)
    const history = loadHistory();
    const nightEntries = history.filter((e: any) => {
      try {
    ...
    res.json({
      date: nightSummary.date || null,
      sessions: nightSummary.sessions || nightEntries.length,
      productive: nightSummary.productive || false,
      energy_avg: nightSummary.energy_avg || 'none',
      shipped: nightSummary.shipped || [],
      summary: nightSummary.summary || (nightEntries.length > 0 ? `${nightEntries.length} night activities` : 'No night workshop data'),
      night_entries: nightEntries.slice(-5)
    });
  } catch (error) {
