...
"
    ...
    help|--help|-h)
        ...
        echo "  intrusive.sh --priority <type>   Bypass mood and pick priority thought directly"
...
import json, random
try:
    with open('$DATA_DIR/thoughts.json') as f:
        thoughts_data = json.load(f)
    ...
    priority_type = '$PRIORITY_TYPE'
    if priority_type == 'human-request':
        candidates = [t for t in all_thoughts if t['id'] in ['ask-opinion', 'ask-preference', 'ask-feedback']]
    elif priority_type == 'system-alert':
        candidates = [t for t in all_thoughts if t['id'] in ['check-projects', 'system-tinker']]
    elif priority_type == 'deadline':
        candidates = [t for t in all_thoughts if t['id'] in ['build-tool', 'upgrade-project']]
    else:
        candidates = all_thoughts[:3]
    ...
    if candidates:
        ...
        print(json.dumps({'error': 'No suitable thoughts found'}))
except Exception as e:
...
model_hint = 'standard'
try:
    with open('$SCRIPT_DIR/model_hints.json') as f:
        hints = json.load(f)
...
genuineness_reason = 'No check performed'
try:
...
print(json.dumps({
    ...
    'genuineness_reason': genuineness_reason,
